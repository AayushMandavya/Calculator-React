import React, { useState} from "react";
import "./App.css";

function App() {
  const [result, setResult] = useState("");

  const ChangeSign = () => {

    if (result.slice(0, 1) === "-") {
      setResult("+".concat(result));
    } else {
      setResult("-".concat(result));
    }

  };

  function handleClick(e) {
    setResult(result.concat(e.target.name));
  }

  function clear() {
    setResult("");
  }

  function calculate() {
    try {
      setResult(eval(result).toString());
    } catch (error) {
      setResult("Error");
    }
  }

  return (
    <div className="Calculator__wrap">
      <from>
        <input type="text" className="Display" value={result}/>
      </from>

      <div className="Calculator__button">
        <button id="clear" onClick={clear}>
          AC
        </button>
        <button id="changeSign" onClick={ChangeSign}>
          +/-
        </button>
        <button name="%" onClick={handleClick}>
          %
        </button>
        <button className="Divide" name="/" onClick={handleClick}>
          /
        </button>
        <button name="7" onClick={handleClick}>
          7
        </button>
        <button name="8" onClick={handleClick}>
          8
        </button>
        <button name="9" onClick={handleClick}>
          9
        </button>
        <button name="*" className="Multiply" onClick={handleClick}>
          &times;
        </button>
        <button name="4" onClick={handleClick}>
          4
        </button>
        <button name="5" onClick={handleClick}>
          5
        </button>
        <button name="6" onClick={handleClick}>
          6
        </button>
        <button name="-" className="Minus" onClick={handleClick}>
          -
        </button>
        <button name="1" onClick={handleClick}>
          1
        </button>
        <button name="2" onClick={handleClick}>
          2
        </button>
        <button name="3" onClick={handleClick}>
          3
        </button>
        <button name="+" className="Plus" onClick={handleClick}>
          +
        </button>
        <button name="0" id="zero" onClick={handleClick}>
          0
        </button>
        <button name="." onClick={handleClick}>
          .
        </button>
        <button id="result" className="Result" onClick={calculate}>
          =
        </button>
      </div>
    </div>
  );
}

export default App;